...
class UserCreateForm(FlaskForm):
    user_id = StringField('사용자이름', validators=[DataRequired(), Length(min=5, max=25), Regexp('^[A-Za-z0-9]*$', message="영문과 숫자만 가능합니다")])
    username = StringField('실명', validators=[DataRequired(), Length(max=25)])
    user_pw1 = PasswordField('비밀번호', validators=[DataRequired(), Length(min=10), EqualTo('user_pw2', '비밀번호가 일치하지 않습니다'), Regexp('^(?=.*[a-z])(?=.*[A-Z])(?=.*\W).+$', message="대문자, 소문자, 기호를 포함해야 합니다")])
